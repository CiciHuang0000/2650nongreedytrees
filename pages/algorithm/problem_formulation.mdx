...
However, direct global optimization of this empirical loss is challenging as it's discontinuous and piecewise-constant for $W$. To overcome the this challenge, a piecewise smooth upper bound on empirical loss is devloped, which make use of the latent variable defined above. The upper bound takes the form:

$$
\mathcal{l}(\Theta^Tf(sgn(Wx)),y) \leq max_{g \in \mathbb{H}^m} (g^TWx+\mathcal{l}(\Theta^Tf(g),y)) - max_{h \in \mathbb{H}^m}(h^TWx)
$$

This upper bound will be used in the algorithm in the next section and will be explained in more detial. 
